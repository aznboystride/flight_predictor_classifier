...
def test():
    ...
    model = model.cuda()
    model.eval()
    for i, (data_point, label_point) in enumerate(zip(test_df_numpy, labels_numpy), 1):
...
def train():
    tensor_file = 'runs/final_train_novalid__'
    train_file = '69225_Inputs_Yes_at_end_FlightTrainNew1.csv'
    test_file = '29668_Inputs_FlightTestNoYNew1.csv'
    train_df_numpy, test_df_numpy, labels_numpy = convertTrainTestToNumpy(train_file, test_file)
    ...
    model = Network(input_dim, output_dim)
    model = model.cuda()
    # model.load_state_dict(torch.load('best_weights_adam_1e4_68p_578L.pth'))
    ...
    if train_split < 1:
        valid_loader = data.DataLoader(dataset=val_set, batch_size=valid_dataset_size, shuffle=True)
        ...
        if train_split < 1:
            with torch.no_grad():
                ...
                if avg_acc_ > best_acc:
                    best_acc = avg_acc_
                    savepath = 'test_best_weights.pth'
...
def __createFeaturesForPd(raw_df):
    ...
    feature_names = ['Passengers', 'Flights', 'Distance', 'Rank']
